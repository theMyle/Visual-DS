...
export default function SimulationTreeChallenge() {
    ...
    return <SimulationTreeCore challenge={challenge} challengeId={challengeId} />;
}
...
function SimulationTreeCore({ challenge, challengeId }: { challenge: ChallengeConfig, challengeId: string }) {
    ...
    const arraysEqual = (left: (string | number | null)[], right: (string | number | null)[]) => {
    ...
    const runBackgroundTestCases = async (code: string): Promise<ChallengeResultSummary[]> => {
        ...
        for (let i = 0; i < additionalCases.length; i++) {
            ...
            const caseIndex = i + 1;
            const caseInputs = testCase.inputs ?? { tree: testCase.input ?? [] };
