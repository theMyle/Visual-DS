...
export default function SimulationQueueChallenge() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const challengeData = await fetchSimulatorChallenge("queue", challengeId);
                setChallenge(challengeData);

                if (isAuthLoaded && isSignedIn) {
                    const progress = await fetchSimulatorProgress("queue", `/simulator/queue/${challengeId}`, getToken);
                    if (progress) {
