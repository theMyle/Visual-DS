...
const reducedHeroVariants: Variants = {
  hidden: { opacity: 0 },
  show: {
    opacity: 1,
    transition: {
...
export default function App() {
  ...
  // Memoize transform values for parallax effects - only create if motion is enabled
  const heroY = useTransform(scrollYProgress, [0, 1], ["0%", "-50%"]);
  const heroOpacity = useTransform(scrollYProgress, [0, 0.5], [1, 0]);
  const cardsY = useTransform(scrollYProgress, [0.3, 1], ["100%", "0%"]);

  // Use simpler animations for reduced motion
  const activeHeroVariants = shouldReduceMotion ? reducedHeroVariants : heroVariants;
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        {/* Animated background elements - optimized with GPU acceleration */}
        {!shouldReduceMotion && (
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <motion.div
              className="absolute top-20 left-10 w-72 h-72 bg-indigo-200 rounded-full mix-blend-multiply opacity-20"
              style={{
                filter: "blur(40px)",
            ...
            />
            <motion.div
              className="absolute bottom-20 right-10 w-72 h-72 bg-purple-200 rounded-full mix-blend-multiply opacity-20"
              style={{
                filter: "blur(40px)",
                willChange: "transform"
              }}
